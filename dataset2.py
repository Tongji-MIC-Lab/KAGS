...
class VISTDataset(Dataset):
    def __init__(self, opt):
        ...
        self.id2word=np.load('./data/id2word_total_12322.npy', allow_pickle=True)[()]
        ...
        for split in ['val', 'test']:
            ...
            with open(os.path.join(ref_dir, '{}_desc_reference.json'.format(split)), 'w') as f:
                json.dump(reference, f)
        ########
        self.keywords=np.load('./story_files/keyfiles/story_keywords_vist_test_encode.npy', allow_pickle=True)[()]
        self.story_ids_test=np.load('./story_files/keyfiles/story_ids_test.npy', allow_pickle=True)[()]
        self.events=np.load('./story_files/keyfiles/event2id_vist_test.npy', allow_pickle=True)[()]
        img_id_all=[]
    ...
    def __getitem__(self, index):
        if self.task == 'story_telling':
            ...
            story_id=self.story_ids_test[index]
            ...
            for i in range(story['length']):
                ...
                feature_fc[i] = np.load(fc_path)# load events id
    ...
    def __len__(self):
        if self.task == 'story_telling':
            return len(self.story_ids_test)
        elif self.task == 'image_captioning':
