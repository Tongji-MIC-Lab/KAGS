...
from dataset2 import VISTDataset
import opts
opt=opts.parse_opt()
...
class Evaler(object):
    ...
    def make_kwargs(self, indices, ids, gv_feat, att_feats, att_mask, fc_feats, keywords, events):
        kwargs = {}
    ...
    def __call__(self, model, rname):
        ...
        predictions = {}
        ##
        txt_root='./prediction_results/predict_'
        ...
        with torch.no_grad():
            ...
            for iter, batch in enumerate(self.test_loader):
                ...
                events=events.view(B1*B2, -1).cuda()
                #events=events.cpu().numpy()
                ids=indices
                kwargs = self.make_kwargs(indices, ids, gv_feat, att_feats, att_mask, fc_feats, keywords, events)
                #print('beam_size is [%d]' %(kwargs['BEAM_SIZE']))
                if kwargs['BEAM_SIZE'] > 1:
                    #print('beam search is 3')
