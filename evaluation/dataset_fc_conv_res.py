...
class VISTDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.task == 'story_telling':
            ...
            ######   for fc and obj features
            ####obj_root='/data/ltp/CODEs/story/features/VIST/BU-SENET154'
            fc_root = '/data/ltp/CODEs/story/AREL-master/DATADIR/resnet_features/fc'
            conv_root='/data/ltp/CODEs/story/AREL-master/DATADIR/resnet_features/conv'
            # load feature
            feature_conv = np.zeros((story['length'], 7, 7, self.opt.feat_size), dtype='float32')
            feature_fc = np.zeros((story['length'], self.opt.feat_size), dtype='float32')
            for i in range(story['length']):
                # load obj feature
#                obj_path = os.path.join(obj_root, self.mode, '{}.npz'.format(story['flickr_id'][i]))
#                obj_feature=np.load(obj_path)['feat']
#                obj_feature=np.array(obj_feature).astype('float32')
#                box_len=obj_feature.shape[0]
#                feature_obj[i][0:box_len]=obj_feature
                conv_path = os.path.join(conv_root, self.mode, '{}.npz'.format(story['flickr_id'][i]))
                fc_path = os.path.join(fc_root, self.mode, '{}.npy'.format(story['flickr_id'][i]))
                feature_conv[i] = np.load(conv_path)['arr_0']
                feature_fc[i] = np.load(fc_path)
                
                
            sample = {'feature_conv': feature_conv, 'feature_fc': feature_fc}
            ...
            sample['index'] = np.int64(story_id)
